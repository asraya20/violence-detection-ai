...
def main():
    ...
    # 🔥 VIDEO INPUT
    VIDEO_PATH = r"C:\Users\Adithya\Desktop\WhatsApp Video 2026-03-26 at 6.02.02 PM.mp4"
    print(f"[INFO] Running on video: {VIDEO_PATH}")
                ...
                if cls == 0:
                    ...
                    if pose_results and pose_results.pose_landmarks:
                        ...
                        if len(sequence_buffer) == 30:
                            prob = predict_sequence(model, np.array(sequence_buffer))
        ...
        if len(person_centers) >= 2:
            for i in range(len(person_centers)):
                for j in range(i + 1, len(person_centers)):
                    dist = np.linalg.norm(np.array(person_centers[i]) - np.array(person_centers[j]))
...
# ---------------- STABLE VIOLENCE ----------------
        ...
        cv2.imshow("Violence Detection - Video Mode", frame)

        if cv2.waitKey(1) & 0xFF == ord('q'):
            break

        # 🔥 simulate real-time (important)
        time.sleep(1/30)
